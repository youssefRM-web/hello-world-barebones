...
import { Pencil, ScanLine, ArrowLeft, Plus, ChefHat, DollarSign, Calendar, Utensils, Tag, Eye } from "lucide-react";
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div className="space-y-2 flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {menuData.created_at ? new Date(menuData.created_at).toLocaleDateString() : "-"}
                    </span>
                    <span className="flex items-center gap-1">
                      <Utensils className="w-4 h-4" />
                      {menuData.menus?.length || 0} {t("categoriesCount")}
                    </span>
                  </div>
                ...
                <Button
                  variant="outline"
                  className="border-[#0A2472] text-[#0A2472] hover:bg-[#0A2472]/10 rounded-full text-sm"
                  onClick={() => navigate("/menu-editing")}
                >
